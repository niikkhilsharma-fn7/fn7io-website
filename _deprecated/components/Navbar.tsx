...
export default function Navbar() {
    ...
    return (
      <div
        className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 ${
          isTablet ? "w-[90%] max-w-[700px]" : "w-[800px]"
        } bg-black rounded-[10px] flex items-center justify-between ${
          isTablet ? "gap-4" : "gap-[60px]"
      ...
      >
        ...
        <button
          ...
          className={`bg-gradient-to-r from-[#ff482ccc] to-[#a245eecc] text-white ${
            isTablet ? "w-[140px] h-[32px] text-base" : "w-[180px] h-[35px] text-lg"
          } rounded-xl leading-6 whitespace-nowrap`}
        >
          Get Started
        </button>
      </div>
  ...
  return (
    <div className={`sticky top-0 z-50 ${isMobile ? "bg-white" : isTablet ? "px-6 bg-white" : "px-10 bg-white"}`}>
      <nav
      ...
      >
        ...
        <div className="flex items-center gap-x-4">
          ...
          <button onClick={() => setMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden text-white cursor-pointer">
          ...
          <div className="absolute top-[85px] right-0 bg-black text-white rounded-2xl p-6 flex flex-wrap flex-col gap-4 z-50 md:hidden w-full transition-opacity duration-300">
            {links.map(({ href, label }) => (
              <Link
                ...
                className={`text-xl cursor-pointer ${router.pathname === href ? "font-bold" : ""}`}
              >
                {label}
              </Link>
            ))}
